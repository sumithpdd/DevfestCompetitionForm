...
export default function AdminPage() {
  ...
  const handlePlaceChange = async (submissionId: string, place: string) => {
    try {
      const submissionRef = doc(db, "DevFestComp2025", submissionId);
      const newPlace = place === "none" ? null : (place as "first" | "second" | "third");
      
      await updateDoc(submissionRef, { place: newPlace });
  ...
  const handleDeleteSubmission = async () => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "DevFestComp2025", submissionToDelete));
