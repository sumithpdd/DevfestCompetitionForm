...
export const getUserProfile = async (uid: string): Promise<UserProfile | null> => {
  const userRef = doc(db, "CompetitionUsers", uid);
  const userSnap = await getDoc(userRef);
...
2. Open `CompetitionUsers` collection
...
4. Checks if user exists in `CompetitionUsers` collection
...
**Fix**:
- Check Firestore `CompetitionUsers` collection
